...
3. **📋 CLI 数组格式**: `cli` 字段使用数组类型 `CLIType[]`（如 `['claude']`、`['codex']`、`['claude', 'codex']`），支持同一命令适配多个 CLI
4. **📋 多 CLI 合并规则**: 当 `.claude/` 和 `.codex/` 目录中存在同名命令时，合并为一条记录，`cli` 设为 `['claude', 'codex']`，描述取更完整的版本
