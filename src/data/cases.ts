...
export const MULTI_CLI_CASES: Case[] = [
  {
    ...
    steps: [
      ...
      { role: 'user', content: '/issue:queue', type: 'command' },
      { role: 'system', content: '┌─ 执行队列 ───────────────────────────────────┐\n│                                              │\n│ 📌 批次 1 (可并行):                         │\n│    └─ Issue-1: 购物车数据模型               │\n│                                              │\n│ 📌 批次 2 (可并行):                         │\n│    ├─ Issue-2: 购物车 API 端点              │\n│    └─ Issue-5: 前端购物车组件               │\n│                                              │\n│ 📌 批次 3 (可并行):                         │\n│    ├─ Issue-3: 商品添加/移除逻辑            │\n│    └─ Issue-6: 结算流程集成                 │\n│                                              │\n│ 📌 批次 4:                                  │\n│    └─ Issue-4: 数量修改与校验               │\n└──────────────────────────────────────────────┘', type: 'response' },
      { role: 'system', content: '💡 现在可以在 Codex 中并行开发:\n\n  窗口 1: codex\n  > /parallel-dev-cycle TASK="实现购物车 API 端点" --auto\n\n  窗口 2: codex\n  > /parallel-dev-cycle TASK="实现前端购物车组件" --auto\n\n  批次 2 的两个任务可以并行开发！', type: 'tip' },
    ],
  ...
  },
  {
    ...
    commands: [
      ...
      { cmd: '/issue:queue', desc: '形成执行队列' },
      { cmd: '/parallel-dev-cycle', desc: 'Codex 并行开发循环' },
    ],
    steps: [
      ...
      { role: 'system', content: '🚀 启动 Codex 并行开发循环...\n\n--- 终端 A (后端核心) ---\n$ codex\n> /parallel-dev-cycle TASK="支付渠道抽象层+状态机+对账" --auto\n\n--- 终端 B (后端支付) ---\n$ codex\n> /parallel-dev-cycle TASK="微信支付+支付回调" --auto\n\n--- 终端 C (前端) ---\n$ codex\n> /parallel-dev-cycle TASK="支付宝集成+对账报表" --auto', type: 'response' },
  ...
  },
  {
    ...
    steps: [
      ...
      { role: 'system', content: '🚀 各开发者启动 Codex 并行开发循环:\n\n开发者 A 的终端:\n$ codex\n> /parallel-dev-cycle TASK="用户模块: 注册/登录/权限/个人中心" --auto\n\n开发者 B 的终端:\n$ codex\n> /parallel-dev-cycle TASK="课程模块: 管理后台/章节/视频" --auto\n\n...(其他开发者类似)\n\n💡 每个模块由4个Agent并行: RA(需求)→EP(规划)→CD(开发)→VAS(验证)', type: 'tip' },
  ...
  },
  {
    ...
    steps: [
      ...
      { role: 'system', content: '💡 批次 2 可拆分到多个 Codex 终端并行开发:\n\n终端 1:\n$ codex\n> /parallel-dev-cycle TASK="断点续传支持" --auto\n\n终端 2:\n$ codex\n> /parallel-dev-cycle TASK="上传进度追踪" --auto\n\n终端 3:\n$ codex\n> /parallel-dev-cycle TASK="服务端接收 API" --auto', type: 'tip' },
  ...
  },
  {
    ...
    steps: [
      ...
      { role: 'system', content: '💡 切换到 Codex 终端并行开发...\n\n--- Codex 终端 1 (后端核心) ---\n$ codex\n> /parallel-dev-cycle TASK="CRDT算法+WebSocket+Redis消息" --auto\n\n--- Codex 终端 2 (数据层) ---\n$ codex\n> /parallel-dev-cycle TASK="MongoDB存储+版本历史" --auto\n\n--- Codex 终端 3 (前端) ---\n$ codex\n> /parallel-dev-cycle TASK="前端编辑器+协作光标+权限控制" --auto', type: 'tip' },
