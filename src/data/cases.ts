...
export const MULTI_CLI_CASES: Case[] = [
  ...
  {
    id: 'MCLI-003',
    title: '深度分析 + Wave Plan 并行实现（v7.0）',
    level: 'multi-cli',
    category: '分析执行',
    scenario: 'Claude Code 深度分析架构问题，输出分析文件，workflow-wave-plan CSV 波浪式并行实现修复',
    commands: [
      { cmd: '/workflow:analyze-with-file', desc: '交互式协作分析，产出分析文件' },
      { cmd: '/workflow-wave-plan', desc: 'CSV Wave 规划执行 - 先勘探再施工' },
    ],
    steps: [
      { role: 'user', content: '/workflow:analyze-with-file "分析支付模块性能瓶颈和架构问题"', type: 'command' },
      { role: 'system', content: '┌─ 交互式协作分析 ────────────────────────────┐\n│ 🔍 Phase 1: Topic Understanding              │\n│ 🧠 分析维度: 架构, 性能, 依赖                 │\n└──────────────────────────────────────────────┘', type: 'response' },
      { role: 'system', content: '📊 Phase 2-4: CLI 探索 + 交互讨论 + 综合结论\n\n  🔴 P0 瓶颈:\n    - calculateFee() 每次调用查 DB (N+1)\n    - 汇率缓存完全缺失\n    - 支付验证串行执行，可并行化\n\n  🟡 架构问题:\n    - PaymentService 职责过多\n    - 缺乏 Circuit Breaker 保护\n    - 重试逻辑散落各处\n\n  📁 结论文件: .workflow/.analysis/ANL-payment/conclusions.json\n  ✅ 分析完成，自动流转到 Wave Plan', type: 'result', highlight: true },
      { role: 'user', content: '/workflow-wave-plan "修复支付模块性能问题"', type: 'command' },
      { role: 'system', content: '┌─ Wave Plan 初始化 ──────────────────────────┐\n│ 📋 模式: 先勘探 (explore.csv) → 再计划 → 后施工 (tasks.csv) │\n└──────────────────────────────────────────────┘', type: 'response' },
      { role: 'system', content: '🌊 Phase 1: 探索波 (3 个 Explore Agent 并行)\n\n  ⚡ 波次 1:\n    ✓ E1: 现有支付代码扫描 → 发现 legacy-pay.ts 耦合严重\n    ✓ E2: 依赖分析 → 找到 3 个 SDK 入口\n    ✓ E3: 测试覆盖现状 → 当前覆盖率 42%\n\n  📁 explore.csv 已更新', type: 'response' },
      { role: 'system', content: '📋 Phase 2-3: 冲突解决 + 执行计划 (tasks.csv)\n\n  Wave 2: T1 - 汇率缓存层 [context: E1+E2]\n  Wave 3: T2 - N+1 查询修复 [context: E2+T1]\n  Wave 3: T3 - 并行验证改造 [context: E1+T1]\n  Wave 4: T4 - Circuit Breaker 集成 [context: E1+T2+T3]', type: 'response' },
      { role: 'system', content: '⚡ Phase 4: Wave 执行\n\n  Wave 2 (T1): 汇率缓存层\n    ✅ ExchangeRateCache.ts 完成\n    ✅ 测试: calculateFee() P99 8ms → 0.3ms\n\n  Wave 3 (T2+T3 并行):\n    ✅ T2: 批量查询重构 → DB 调用减少 87%\n    ✅ T3: Promise.all() 重构 → 验证延迟 180ms → 45ms\n\n  Wave 4 (T4):\n    ✅ opossum Circuit Breaker 集成完成', type: 'response' },
      { role: 'system', content: '✅ Wave Plan 完成！\n\n📊 性能改善:\n   - calculateFee()  : 8ms → 0.3ms (-96%)\n   - processPayment(): 180ms → 45ms (-75%)\n   - DB 调用数       : -87%\n\n📁 产出: .workflow/.wave-plan/wp-payment-{date}/\n   - explore.csv (探索记录，上下文源)\n   - tasks.csv (执行记录，结果追踪)\n   - context.md (完整报告)\n   - discoveries.ndjson (共享发现板)\n\n💡 analyze-with-file 深度分析定位问题\n   workflow-wave-plan CSV 上下文链路实现任务传递', type: 'result', highlight: true },
    ],
    tips: [
      'analyze-with-file 深度分析，产出结构化结论文件',
      'workflow-wave-plan 用 CSV 记录探索和执行上下文',
      'context_from 实现跨阶段上下文传递 (E*→T*)',
      'Wave 并行执行，依赖任务自动排序',
      '适合性能优化、架构重构等复杂探索性任务',
    ],
  },
