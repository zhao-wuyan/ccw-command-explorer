...
export const LEVEL_1_CASES: Case[] = [
  ...
  },
  {
    ...
    tips: ['自动诊断问题原因', '适合单文件、已定位的明确 Bug'],
  },
];
...
export const LEVEL_3_CASES: Case[] = [
  {
    ...
    level: 3,
    category: 'TDD开发',
    scenario: '用 workflow-tdd-plan 实现支付模块',
    commands: [
    ...
    steps: [
      ...
      { role: 'system', content: '📋 Phase 5 生成 Red-Green-Refactor 任务链:\n\n  🔴 RED-001: 支付接口测试 (先写失败测试)\n  🟢 GREEN-001: 实现 IPaymentGateway (让测试通过)\n  🔵 REFACTOR-001: 提取公共逻辑\n\n  🔴 RED-002: 微信支付测试\n  🟢 GREEN-002: WechatPaymentGateway.ts\n  🔵 REFACTOR-002\n\n  🔴 RED-003: 支付宝测试\n  🟢 GREEN-003: AlipayGateway.ts\n  🔵 REFACTOR-003', type: 'response' },
      { role: 'system', content: '✅ TDD 任务链已生成！\n\n📁 产出:\n   - .workflow/tdd-plan/tasks/ (9个任务)\n\n▶️ 开始执行 Red-Green-Refactor 循环...', type: 'note' },
      { role: 'system', content: '🔴→🟢→🔵 执行 Cycle 1: 测试失败 ✓ → 实现通过 ✓ → 重构完成 ✓\n🔴→🟢→🔵 执行 Cycle 2: 测试失败 ✓ → 实现通过 ✓ → 重构完成 ✓\n🔴→🟢→🔵 执行 Cycle 3: 测试失败 ✓ → 实现通过 ✓ → 重构完成 ✓', type: 'response' },
      { role: 'system', content: '✅ TDD 开发完成！\n\n📁 新增文件:\n   - src/payment/IPaymentGateway.ts\n   - src/payment/WechatPaymentGateway.ts\n   - src/payment/AlipayGateway.ts\n   - tests/payment/*.test.ts (3个)\n\n📊 测试覆盖率: 94%\n🔴→🟢→🔵 严格遵循 Red-Green-Refactor', type: 'result', highlight: true },
    ],
    tips: ['v7.0 新命令，6阶段TDD规划', 'Red-Green-Refactor 严格执行', '先写测试再写实现，保证质量'],
  },
  {
    id: 'L3-002',
    title: '复杂功能开发（规划+执行）',
    ...
    steps: [
      ...
      { role: 'system', content: '┌─ 5阶段规划 ─────────────────────────────────┐\n│ 📋 Phase 1: 创建会话 WFS-OAuth2-2026-01      │\n│ 📋 Phase 2: 收集项目上下文                   │\n│ 📋 Phase 3: AI 分析需求                      │\n│ 📋 Phase 4: 澄清问题                         │\n│ 📋 Phase 5: 生成任务文件                     │\n└──────────────────────────────────────────────┘', type: 'response' },
    ...
    tips: ['5阶段规划生成详细文档', '澄清问题确保需求准确', '适合复杂功能、多模块开发'],
  },
  {
    id: 'L3-003',
    title: '代码审查 + 自动修复',
    ...
    commands: [
      { cmd: '/review-cycle', desc: '多维度代码审查与修复' },
    ],
    steps: [
      { role: 'user', content: '/review-cycle', type: 'command' },
      { role: 'system', content: '┌─ 代码审查 (7维度) ──────────────────────────┐\n│ 📊 审查范围: 当前会话所有改动  12 个文件     │\n└──────────────────────────────────────────────┘', type: 'response' },
      { role: 'system', content: '  ✅ 正确性: 通过\n  ⚠️ 性能: 发现 2 个问题\n  ❌ 安全性: 发现 1 个问题\n  ⚠️ 可维护性: 发现 1 个问题', type: 'response' },
      { role: 'system', content: '🔴 安全: src/auth/oauth.ts:45 — state 参数未验证 (CSRF 风险)\n🟡 性能: src/auth/session.ts:23 — 每次请求查询数据库\n🟡 可维护: src/auth/providers.ts — GitHub/Google 逻辑重复', type: 'note' },
      { role: 'system', content: '是否自动修复？', type: 'choice' },
      { role: 'user', content: '是', type: 'command' },
      { role: 'system', content: '  ✅ 修复 CSRF 安全问题... ✓\n  ✅ 添加缓存层... ✓\n  ✅ 抽取公共方法... ✓', type: 'response' },
      { role: 'system', content: '✅ 审查问题已全部修复！', type: 'result', highlight: true },
    ],
  ...
  },
  {
    ...
    steps: [
      ...
      { role: 'system', content: '📋 提出假设:\n  [1] 心跳超时  [2] 网络波动  [3] Nginx 代理超时  [4] 资源泄露', type: 'note' },
      { role: 'user', content: '3', type: 'command' },
      { role: 'system', content: '✅ 发现: nginx.conf proxy_read_timeout=60s\n⚠️ 问题: WebSocket 心跳 30s，但 60s 无数据断开\n\n修复方案 A: 增加超时  B: 客户端 ping/pong', type: 'note' },
      { role: 'user', content: 'B', type: 'command' },
      { role: 'system', content: '✅ 调试完成！已实施 ping/pong 修复\n📁 报告: .workflow/.debug/DBG-WS/debug-report.md', type: 'result', highlight: true },
    ],
    tips: ['科学的假设-验证方法', '自动记录调试过程', '文档化调试探索过程'],
  },
  {
    id: 'L3-005',
    title: '测试生成与修复循环',
    ...
    steps: [
      ...
      { role: 'system', content: '📊 分析代码覆盖: src/auth/**\n🔍 生成测试计划: 3 个场景，8 个测试用例', type: 'response' },
      { role: 'system', content: '🧪 第 1 轮: 6/8 通过 (75%)\n  ✗ token 过期处理\n  ✗ session 销毁逻辑', type: 'response' },
      { role: 'system', content: '🔧 自动修复 2 个问题...', type: 'note' },
      { role: 'system', content: '🧪 第 2 轮: 8/8 通过 (100%)\n\n✅ 全部测试通过！覆盖率: 87%', type: 'result', highlight: true },
    ],
    tips: ['自动生成多层测试', '失败自动修复', '循环直到通过率达标'],
  },
  {
    id: 'L3-006',
